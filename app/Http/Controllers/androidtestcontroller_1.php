...
class AndroidTestController extends Controller {
    ...
    public function login() {
       return json_encode(
		    [
			'status' => 'success',
			'business_name' => 'Africa investment',
			'phone_number' => '0789434343',
			'business_id' => 1,
			'username' => 'africa',
			'profile_link' => 'http://karibusms.com/default.png';
		    ]
	    ); 
	$this->log();
	...
	if (!empty($results)) {
	  ...
	  //  $login = new LoginController();
	   // $device = request('manufacturer') . ' ' . request('model');
	    ...
	    return json_encode(['status' => 'warning', 'message' => 'wrong phone number or password']);
	}
    }
